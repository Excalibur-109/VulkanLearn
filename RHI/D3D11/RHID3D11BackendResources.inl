RHIBuffer RHID3D11Backend::createBuffer(const RHIBufferDesc& desc) {
    if (!isInitialized()) {
...
RHITexture RHID3D11Backend::createTexture(const RHITextureDesc& desc) {
    if (!isInitialized()) {
        throw std::runtime_error("RHID3D11Backend is not initialized");
    }
    ...
    if (format == DXGI_FORMAT_UNKNOWN) {
        throw std::runtime_error("RHITextureDesc::format is not supported by D3D11");
    }
    ...
    if (desc.dimension == RHITextureDimension::Texture1D) {
        ...
        textureDesc.BindFlags = toTextureBindFlags(desc.usage, RHIMemoryUsage::GpuOnly);
        textureDesc.MiscFlags = RHIHasAny(desc.flags, RHITextureCreateFlags::GenerateMips) ? D3D11_RESOURCE_MISC_GENERATE_MIPS : 0;
    ...
    } else if (desc.dimension == RHITextureDimension::Texture3D) {
        ...
        textureDesc.Usage = D3D11_USAGE_DEFAULT;
        textureDesc.BindFlags = toTextureBindFlags(desc.usage, RHIMemoryUsage::GpuOnly);
        if (RHIHasAny(desc.flags, RHITextureCreateFlags::CubeCompatible)) textureDesc.MiscFlags |= D3D11_RESOURCE_MISC_TEXTURECUBE;
        if (RHIHasAny(desc.flags, RHITextureCreateFlags::GenerateMips)) textureDesc.MiscFlags |= D3D11_RESOURCE_MISC_GENERATE_MIPS;
...
RHITextureView RHID3D11Backend::createTextureView(const RHITextureViewDesc& desc) {
    if (!isInitialized()) {
        throw std::runtime_error("RHID3D11Backend is not initialized");
    }
...
RHISampler RHID3D11Backend::createSampler(const RHISamplerDesc& desc) {
    if (!isInitialized()) {
        throw std::runtime_error("RHID3D11Backend is not initialized");
    }
...
static std::vector<std::byte> compileHlsl(const RHIShaderDesc& desc) {
    ...
    if (profile.empty()) {
        throw std::runtime_error("RHIShaderDesc::stage is not supported by D3D11");
    }
...
RHIShader RHID3D11Backend::createShaderModule(const RHIShaderDesc& desc) {
    if (!isInitialized()) {
        throw std::runtime_error("RHID3D11Backend is not initialized");
    }
...
RHIBindGroupLayout RHID3D11Backend::createBindGroupLayout(const RHIBindGroupLayoutDesc& desc) {
...
RHIBindGroup RHID3D11Backend::createBindGroup(const RHIBindGroupDesc& desc) {
    if (!isInitialized()) {
        throw std::runtime_error("RHID3D11Backend is not initialized");
    }
    ...
    for (const RHIResourceBinding& binding : desc.bindings) {
        ...
        if (binding.type == RHIBindingType::UniformBuffer) {
            ...
            throw std::runtime_error("RHIResourceBinding type is not supported by D3D11");
        }
...
RHIPipelineLayout RHID3D11Backend::createPipelineLayout(const RHIPipelineLayoutDesc& desc) {
...
RHIPipelineCache RHID3D11Backend::createPipelineCache(const RHIPipelineCacheDesc& desc) {
