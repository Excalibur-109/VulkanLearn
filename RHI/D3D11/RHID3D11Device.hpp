...
/// Direct3D 11 后端的统一 RHI 设备。
/// 公开头文件不包含原生 API 类型，具体驱动对象隐藏在 Impl 中。
class RHID3D11Device final : public RHIDevice {
public:
    RHID3D11Device();
    ~RHID3D11Device() override;

    RHID3D11Device(const RHID3D11Device&) = delete;
    RHID3D11Device& operator=(const RHID3D11Device&) = delete;
