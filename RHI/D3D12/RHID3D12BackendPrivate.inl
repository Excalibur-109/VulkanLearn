...
// Impl 是 RHID3D12Backend 的状态仓库。D3D12 的核心对象都是 COM 对象，用 ComPtr 管理生命周期。
// Descriptor 不是 COM 对象，只是 descriptor heap 中的一个槽位，所以用 CpuDescriptor 保存 CPU handle。
struct RHID3D12Backend::Impl {
    ...
    struct SwapchainResource {
        ...
        RHIUInt32 currentImage = 0;
    };

    RHID3D12NativeHandles native{};
    RHID3D12BackendDesc initDesc{};
...
static RHICapabilities makeCapabilities(IDXGIAdapter1* adapter, D3D_FEATURE_LEVEL featureLevel) {
    ...
    caps.maxPushConstantSize = D3D12_MAX_ROOT_COST * sizeof(RHIUInt32);
