bool D3D12Renderer::recordAndSubmitFrame(const FramePacket& packet, std::string* errorMessage) {
    try {
        ...
        for (const BufferUploadDesc& upload : packet.uploads.buffers) {
            ...
            if (upload.destinationOffset + upload.data.size() > buffer->desc.size) {
                throw std::runtime_error("FramePacket buffer upload range exceeds destination buffer size");
            }
        ...
        if (!packet.workloads.empty()) {
            throw std::runtime_error(
                "D3D12 FramePacket workload recording is not implemented yet: "
...
bool D3D12Renderer::submitFrame(const FramePacket& packet, std::string* errorMessage) {
...
void D3D12Renderer::waitIdle() const noexcept {
